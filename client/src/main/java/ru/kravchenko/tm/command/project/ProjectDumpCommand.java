...
public class ProjectDumpCommand extends AbstractCommand {
    ...
    public void execute() throws AccessForbiddenException_Exception {
        ...
        System.out.println("***Project Dump Command***");
        ...
        switch (userSelect) {
            case "1":
                projectEndpoint.saveDateSerializ(session);
                break;
            case "2-1":
                projectEndpoint.saveDateJAXBtoXML(session);
                break;
            case "2-2":
                projectEndpoint.saveDateJAXBtoJson(session);
                break;

//            case "3-1":
//                projectEndpoint.saveDateOMtoXML(session);
//                break;

            case "3-2":
                projectEndpoint.saveDateOMtoJson(session);
                break;
